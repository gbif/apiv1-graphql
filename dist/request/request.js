...
const agentMapping = [{
  startsWith: '/v1/occurrence/',
  requestAgent: requestAgents.occurrence
}, {
  startsWith: '/v1/species/',
  requestAgent: requestAgents.species
}];

function requestWrapper(options) {
  const path = url.parse(options.url).path; // console.log(path);
...
requestWrapper.get = url => requestWrapper({
  url
});
