...
exports.enumTypeDefs = enumTypeDefs;
...
async function loadEnums() {
  const types = await getDate(`${_config.gbifApi}/enumeration/basic`);
  const expectedTypes = types.filter(x => skipTypes.indexOf(x) == -1);
  const enums = await Promise.all(expectedTypes.map(type => getDate(`${_config.gbifApi}/enumeration/basic/${type}`)));

  const enumMap = _lodash.default.zipObject(expectedTypes, enums);

  return enumMap;
}

async function getDate(url) {
  const res = await (0, _requestretry.default)({
    url,
    json: true,
    maxAttempts: 1
  });
...
async function enumTypeDefs() {
  ...
  const enums = await loadEnums(); //map enums to schema definitions
